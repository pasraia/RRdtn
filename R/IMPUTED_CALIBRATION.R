                               ...
                               eval.metric)
{
  ...
  if (!is.null(spec_for_imputation)) {
    all_models_NN <- ENFA_output[!names(ENFA_output) %in%
                                   spec_for_imputation]
  }
  if (!is.null(clust))
      ...
      output <- foreach(i = 1:nsim, .packages = c("RRphylo",
                                                  "Rphylopars", "ape")) %dopar% {
                                                    ...
                                                    if (is.null(spec_for_imputation)) {
                                                      ...
                                                      data_final <- dat_tot[spec_for_imputation]
                                                    }
                                                  }
      stopCluster(cl)
      closeAllConnections()
    ...
    if (evaluate) {
        ...
        if (nrow(aa) >= 10) {
          ...
          if (parallel) {
            ...
            EVAL <- foreach(s = 1:nsim, .packages = c("dismo",
                                                      "ecospat", "PresenceAbsence"),
                            .export = c("mahasuhab.custom", "aa",
                                        "boot_reps")) %dopar% {
      ...
      output <- mapply(function(x, y) {
        if (!is.null(y))
          x$co <- x$co[as.numeric(gsub("swap_", "",
                                       y$swap))]
        return(x)
    ...
    output <- mapply(function(jj, kk, ll, nam) {
      if (!is.null(kk)) {
        ...
        jj$evaluation <- jj$evaluation[, -grep("swap",
                                               colnames(jj$evaluation))]
      }
      ...
      list(jj)->mm
      names(mm)<-nam
      save(mm, file = paste0("ENphylo_imputed_models/", nam,
                             "/output_model.RData"))
      jj
